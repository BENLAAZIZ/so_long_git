...
/*   Updated: 2024/03/23 01:56:59 by hben-laz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int arg_c, char **arg_v)
{
	...
	if (arg_c != 2)
		return (ft_print_error("arg"), 0);
	if (handel_content(get_buffer(arg_v, &data), &data) == 1)
		return (ft_print_error("map"), 0);
	data.split = ft_split(get_buffer(arg_v, &data), '\n');
	if (!data.split)
		return (free(data.buffer), 0);
	ft_init(&data);
	if ((handel_border(data.split, data.width, data.height)) == 1)
		return (free_split_buffer(&data), destroy_all(&data), 0);
	cord_player(&data);
	data.p.coi_copy = data.p.coi;
	if (fload_fill(&data, data.y_p, data.x_p, &data.p.a))
		return (free_split_buffer(&data),
			ft_print_error("map"), destroy_all(&data), 0);
